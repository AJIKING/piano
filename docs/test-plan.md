...
| `LibraryStore` | ユーザー楽譜(自作・編集)の永続化 | デバイスストレージ / インメモリ fake |
| `AudioEngine` | 発音・再生(Tone.js 相当の副作用) | flutter_midi_pro + SoundFont / 呼び出しを記録するだけの fake(音を鳴らさない) |

通信がないため `ApiClient` は不要。`Random` 依存もない(プロトタイプの旋律はデモ固定で、シャッフル要素がない)。日時に依存する機能(習得度・最終練習日時)は廃止したため `Clock` 境界は持たない。再生のタイミングは実時間ではなく `fake_async` で `Timer` を進めて検証する。
