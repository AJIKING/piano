...
| 再生スケジュール(`PlaybackSchedule` / `PracticeController`) | 旋律 + bpm から (発音時刻, 音高, 音価) のイベント列を生成 / 末尾+余韻で停止 / テンポ変更の反映(すべて fake_async で実時間を使わない) |
| メトロノーム(`PlaybackSchedule`) | 1 拍ごとのクリック / 拍頭の強拍判定(3 拍子) |
...
| ライブラリ状態(`LibraryController`) | 収録曲+ユーザー曲の結合 / featured は一覧から除外 / 新規作成・編集保存・練習記録が永続化される / 練習完了で習得度+最終練習日時が更新 / snapshot 往復・旧データ互換(featured 補完) |
...
- 「今練習中」カード(`NowPracticingCard`)。
- 鍵盤の 1 オクターブ(`PianoKeyboard`)※UI 実装時。
- 譜面の数小節(`ScoreView`)※UI 実装時。
...
スケルトン段階では `NowPracticingCard` の golden を 1 件用意し、golden ハーネス(setup / tag / skip / baseline 運用)を通す。鍵盤・譜面の golden は対応ウィジェットの実装時に追加する。
