...
ピアノの練習と楽譜づくりを 1 アプリで行う Flutter アプリ「ピアノノート」。収録曲・自作曲の譜面を見ながら鍵盤で弾き、テンポ可変で再生し、楽譜エディタで音符を編集する。オフライン完結・バックエンド/認証なし。

> 表示名・ストア名は「ピアノノート」。一方、Dart パッケージ名 `etude` とコード識別子(`EtudeApp` / `EtudeTheme` / `EtudeColors` など)は旧称「Étude」由来のまま据え置く(改名は import への波及が大きく、ユーザーに見えないため)。bundle id も `jp.co.plaria.piano` のまま。
